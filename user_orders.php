...
// Deklaracja i przypisanie zmiennym zawartosci z sesyjnych:
$nick = $_SESSION['user'];
$name = $_SESSION['name'];
...
// Poprawne zalogowanie sie do bazy danych:
...
<head>
    ...
    <title>Vears - Twoje zamówienia</title>
...
<?php } ?> 
    ...
    <section class="section-user">
        ...
        <div class="section-user__options">
            <a href="./zalogowany.php">Dane i adres</a>
            <a href="./user_orders.php" class="active">Zamówienia</a>
            <a href="./favourites.php" >Ulubione</a>
            <a href="./cart.php" >Koszyk</a>
            <a href="./logout.php">wyloguj</a>
        </div>

        <!-- TABELA Z WYKAZEM HISTORII ZAMOWIEN KLIENTA -->
        <div class="section-user__box">
            <p class="section-user__title2 lower-margin3">Lista zakupionych przez Ciebie produktów.</p>
            <div class="order-box">
            <?php
                    $result_in = mysqli_query($polaczenie2, $sql_in);
                    $ile = mysqli_num_rows($result_in);
                        
                    for ($i = 1; $i <= $ile; $i++){
                      $row_in = mysqli_fetch_assoc($result_in);
                      $order_id = $row_in['order_id'];
                      $order_date = $row_in['order_date'];
                      $order_status = $row_in['order_status'];
                        
                    // Tutaj wyswietla sie dane poszczegolnego zamowienia:
                      echo '<div class="tab-space"></div>';
                      echo '<table class="order-table"><tr><td>Numer zamówienia: <p>' .$order_id . '</p></td>';
                      echo '<td>Data złożenia zamówienia:<p>' .$order_date . '</p></td>';
                      echo '<td>Status przesyłki:<p>' .$order_status . '</p></td></tr></table>';

                      // Pobieranie zakupionych produktow do tego numeru zamowienia:
                      $sql_in = "SELECT * FROM ordered_items WHERE order_id = '$order_id'";
                        
                      $result_in_B = mysqli_query($polaczenie2, $sql_in);
                      $ileB = mysqli_num_rows($result_in_B);
                        
                      for ($y = 1; $y <= $ileB; $y++){
                        $row_in_B = mysqli_fetch_assoc($result_in_B);
                        $product_id = $row_in_B['product_id'];
                        $ordered_item_size = $row_in_B['ordered_item_size'];
                        $ordered_pieces = $row_in_B['ordered_pieces'];
                        $ordered_current_price = $row_in_B['ordered_current_price'];
                        
                        // Dodatkowo siega sie do tabeli products po zdjecie i nazwe produktu:
                        $sql_in = "SELECT nameProduct, pictureA FROM products WHERE idproduct = '$product_id'";
                        $result_in_C = mysqli_query($polaczenie2, $sql_in);
                        $ileC = mysqli_num_rows($result_in_C);
                            
                        for ($z = 1; $z <= $ileC; $z++){
                          $row_in_C = mysqli_fetch_assoc($result_in_C);
                          $nameProduct = $row_in_C['nameProduct'];
                          $pictureA = $row_in_C['pictureA'];

                          // Tutaj dopiero powinny generowac sie produkty:
                          echo<<<END
                          <div class="order-box__order-product">
                            <div class="item-block">
                              <figure>
                                <div class="img-box">
                                    <img src="$pictureA" alt="fotografia zamawianego produktu">
                                </div>
                              </figure>
                              <div class="info-about">
                                <p><strong>$nameProduct</strong></p>
                                <p>Rozmiar: $ordered_item_size</p>
                                <p>Ilość x $ordered_pieces</p>
                                <p>Cena produktu: $ordered_current_price zł</p>
                              </div>
                            </div>
                          </div>
END;
