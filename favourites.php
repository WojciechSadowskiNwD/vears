...
// Czy jest zalogowany, jesli nie przekieruj na strone glowna:
...
try{
    ...
    if($polaczenie->connect_errno!=0){
        ...
        if(isset($_POST['search-order'])){
            ...
            //B. Sprawdzenie poprawnosci szukanego numeru telefonu:
...
if( $polaczenie2->connect_errno!=0 ){
    ...
    if($polaczenie2 == true){
        $now_id = $_SESSION['id'];
        $sql_in = "SELECT * FROM favourites WHERE user_id = '$now_id'";
    }
}

// Usuwanie polubionego produktu z bazy danych:
if(isset($_POST['remove_fav'])){
    $remove_fav = $_POST['remove_fav'];

    if($polaczenie2->query("DELETE FROM favourites WHERE favourite_product_id ='$remove_fav' AND user_id='$now_id'"));
}
?>
<!DOCTYPE html>
<html lang="pl">
    ...
    <main>
      <section class="section-user">
        ...
        <div class="section-user__options">
            ...
            <a href="./favourites.php" class="active">Ulubione</a>
            <a href="./cart.php" >Koszyk</a>
            <a href="./logout.php">wyloguj</a>
        </div>
        <div class="section-user__box">
            <p class="section-user__title2">Lista polubionych przez Ciebie produktów.</p>
            <div class="order-box">
             <?php
                $sql_in = "SELECT * FROM favourites WHERE user_id='$now_id'";
                ...
                for ($i = 1; $i <= $ile; $i++){
                    ...
                    $product_id = $row_in['favourite_product_id'];
                    $user_id = $row_in['user_id'];
echo<<<END
                <div class="remove-box">
                    <form method="post">
                    <div class="delete-btn position-trash">
                      <i class="fa-solid fa-trash-can">
                          <input type="hidden" name="remove_fav" value="$product_id"/>
...
END;
                    ...
                    // Dodatkowo pobierane sa zdjecie i nazwa produktu z tabeli products:
                    $sql_in = "SELECT * FROM products WHERE idproduct = '$product_id'";
                    ...
                    for ($z = 1; $z <= $ileC; $z++){
                      ...
                      $price = $row_in_C['newPrice'];
                      $pictureA = $row_in_C['pictureA'];
                      
                      // Generowanie sie blokow produktow:
                      echo<<<END
                      <div class="order-box__order-product">
                        <div class="item-block">
                          <figure>
                            <div class="img-box">
                            <a href="./product_details.php?idproduct=$product_id" target="_blank">
                                <img src="$pictureA" alt="fotografia produktu"></a>
                            </div>
                          </figure>
                          <div class="info-about">
                            <p><strong>$nameProduct</strong></p>
                            <p>Cena produktu: $price zł</p>
                          </div>
END; ?>
                        </div>
                      </div>
